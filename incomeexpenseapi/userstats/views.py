...
class ExpenseSummaryStats(APIView):
    ...
    def get(self, request):
        ...
        expenses = Expense.objects.filter(owner=request.user, date_gte=ayear_ago, date__lte=todays_date)
...
class IncomeSourcesSummaryStats(APIView):
    ...
    def get(self, request):
        todays_date = datetime.date.today()
        ayear_ago = todays_date - datetime.timedelta(days=30*12)
        income = Income.objects.filter(owner=request.user, date_gte=ayear_ago, date__lte=todays_date)
